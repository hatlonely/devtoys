...
func TestBase64TextApp_Base64Encode(t *testing.T) {
	...
	Convey("TestBase64TextApp_Base64Encode", t, func() {
		Convey("test std", func() {
			val, err := app.encode("hello world", "std", true)
			So(err, ShouldBeNil)
			So(val, ShouldEqual, "aGVsbG8gd29ybGQ=")
		})

		Convey("test url", func() {
			val, err := app.encode("hello world", "url", true)
			So(err, ShouldBeNil)
			So(val, ShouldEqual, "aGVsbG8gd29ybGQ=")
		})

		Convey("test raw std", func() {
			val, err := app.encode("hello world", "std", false)
			So(err, ShouldBeNil)
			So(val, ShouldEqual, "aGVsbG8gd29ybGQ")
		})

		Convey("test raw url", func() {
			val, err := app.encode("hello world", "url", false)
			So(err, ShouldBeNil)
			So(val, ShouldEqual, "aGVsbG8gd29ybGQ")
		})
	})
...
func TestBase64TextApp_Base64Decode(t *testing.T) {
	...
	Convey("TestBase64TextApp_Base64Decode", t, func() {
		Convey("test std", func() {
			val, err := app.decode("aGVsbG8gd29ybGQ=", "std", true)
			So(err, ShouldBeNil)
			So(val, ShouldEqual, "hello world")
		})

		Convey("test url", func() {
			val, err := app.decode("aGVsbG8gd29ybGQ=", "url", true)
			So(err, ShouldBeNil)
			So(val, ShouldEqual, "hello world")
		})

		Convey("test raw std", func() {
			val, err := app.decode("aGVsbG8gd29ybGQ", "std", false)
			So(err, ShouldBeNil)
			So(val, ShouldEqual, "hello world")
		})

		Convey("test raw url", func() {
			val, err := app.decode("aGVsbG8gd29ybGQ", "url", false)
			So(err, ShouldBeNil)
			So(val, ShouldEqual, "hello world")
		})
	})
